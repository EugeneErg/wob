...
export class SurfaceMesher {
  ...
  splat(sim, typeId, R){
    ...
    for (let i=0;i<n;i++){
      ...
      if (!(sd<R)) continue;
      const s0 = sd>0 ? sd : 0;
      const gx=xi-2*s0*bnx[i], gy=yi-2*s0*bny[i];
      // Зеркало имеет смысл, только если попало В ТВЁРДОЕ. Стенка тоньше
      // глубины отражения — и призрак вылезает наружу, рисуя воду по ту
      // сторону преграды.
