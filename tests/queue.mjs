...
// --- версии в теле нет ---
//
// Здесь проверялось обратное: что очередь подставляет номер версии истории.
// Версия черновика снята, и проверка перевёрнута намеренно. Она защищала от
// двух рук, сохраняющих карту главы целиком, но карта целиком и была неверной
// единицей записи. Черновик правится по одному объекту: разные точки не
// пересекаются, одна и та же сходится к последней правке — и угадывать номер
// больше не нужно никому.
q.enqueue({ storyId: 'story-1', method: 'put', path: '/api/stories/story-1/levels/lvl-1', body: { name: 'B' } })
await settle()
check('очередь ничего не подставляет в тело', sent[1].body.version === undefined,
  JSON.stringify(sent[1].body))
check('и шлёт ровно то, что просили', sent[1].body.name === 'B')
