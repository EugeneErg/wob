...
for (let t = 0; t < 30; t++) {
  ...
  const walkers = balls().filter((b) => b.rt.state === 'walk').length
  const top = Math.min(...built.map((b) => b.rt.p.y))
  if (t % 5 === 0 || w.physics.links.length !== n0) console.log(`  t=${t + 1}c связей ${w.physics.links.length}, построено ${built.length}, ползут ${walkers}, верх ${top.toFixed(0)}, макс натяжение ${tmax.toFixed(0)}`)
}
