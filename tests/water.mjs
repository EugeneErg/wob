...
const stats = (ph) => {
  ...
  for (const p of ph.points) {
    const s = Math.hypot(p.x - p.px, p.y - p.py) * 120
...
// ----------------------------------------------- жидкость не держит уступ
{
  ...
  // трения покоя, значит уступ обязан растечься. Это оказалось главной
  // проверкой всей среды: при плотной укладке частиц он стоит вечно.
  const ph = new Physics({ gravity: { x: 0, y: 1800 } })
  trough(ph)
  const water = ph.fluid.addPhase({ spacing: 11 })
  ...
  console.log('  при плотной укладке (расталкивание в целый шаг) она стоит здесь вечно: 18 px и через двенадцать секунд')
}
...
// ------------------------------------------------------------- углы и берега
{
  ...
  console.log('  нехватка соседей у стенки живёт по обе стороны границы: поднимаем и камень, и воду вплотную к нему')
